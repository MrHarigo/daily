...
export const useCalendarStore = create<CalendarState>((set, get) => ({
  ...
  fetchHolidays: async (year) => {
    ...
    try {
      set({ isLoading: true });
      const holidays = await api.get<Holiday[]>(`/calendar/holidays/${year}`);
      set((state) => ({
  ...
  fetchDayOffs: async () => {
    try {
      const dayOffs = await api.get<DayOff[]>('/calendar/day-offs');
      set({ dayOffs });
    } catch (error) {
  ...
  addDayOff: async (date, reason) => {
    try {
      const dayOff = await api.post<DayOff>('/calendar/day-offs', { date, reason });
      set((state) => ({
  ...
  removeDayOff: async (date) => {
    try {
      await api.delete(`/calendar/day-offs/${date}`);
      set((state) => ({
